...
public abstract class Girl {
	...
	private Boy bf;
	private ArrayList<String> exbfs;
	 ...
	 * @return
	 */
	public Boy findBF(ArrayList<Boy> allboys) {
		int s = this.selection_criteria;
		if(s==1)
			allboys.sort(Comparator.comparing(Boy::getAttractiveness));
		else if(s==2)
			allboys.sort(Comparator.comparing(Boy::getBudget));
		else
			allboys.sort(Comparator.comparing(Boy::getIntelligence));
		Collections.reverse(allboys);
		Boy temp = null;
		for(Boy b : allboys) {
	...
	public boolean isExBF(Boy b) {
		if(exbfs.size()==0)
			return false;
		String t = exbfs.get(exbfs.size() - 1);
	...
	public ArrayList<String> getExbfs() {
		return exbfs;
	}

	public void setExbfs(ArrayList<String> exbfs) {
		this.exbfs = exbfs;
	}
